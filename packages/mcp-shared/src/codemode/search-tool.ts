...
function parseHelperCallExpression(
	expr: string,
): { fnName: string; argsStr: string } | null {
	...
	if (expr.slice(pos + 1).trim() !== "") return null;
...
function interpretSearchCode(code: string, helpers: OpenApiHelpers): unknown {
	...
	if (helperCall) {
		const fnMap: Record<string, (...a: unknown[]) => unknown> = {
